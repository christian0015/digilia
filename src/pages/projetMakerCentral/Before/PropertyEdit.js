...
const App = (e) => {
  ...
  const storedComponents = JSON.parse(localStorage.getItem('components')) || null;
  const [components, setComponents] = useState(storedComponents);

  const [storedSelected, setStoredSelected] = useState(JSON.parse(localStorage.getItem('selectedComponent'))|| null);
  const [selectedComponent, setSelectedComponent] = useState(JSON.parse(localStorage.getItem('selectedComponent')));
  useEffect(() => {
    setStoredSelected(JSON.parse(localStorage.getItem('selectedComponent'))|| null);
    // setSelectedComponent(storedSelected);
  }, [setInterval(() => {}, 1000)]);
...
useEffect(() => {
  ...
  const reflesh= ()=>{
    console.log("Avant", selectedComponent);
    setSelectedComponent(JSON.parse(localStorage.getItem('selectedComponent'))|| null);
  }
...
const PropertyEditor = () => {
  if (!selectedComponent) return <><div>Sélectionnez un composant pour modifier ses propriétés</div><div><button onClick={reflesh()}>Reflesh</button></div></>;
  ...
  return (
    <div>
      <div>
        {selectedComponent.type === "container" || selectedComponent.type === "header" && (
          <div>
            <h4>Actions</h4>
            <button
